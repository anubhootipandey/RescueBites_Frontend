...
import { ChefHat, Sparkles, Clock, Users, Utensils, ArrowRight, Star, Heart, Zap, Award, TrendingUp, AlertCircle, BookOpen, Trash2, Search, Calendar, X, Settings, Save, RefreshCw, Download, FileText, Printer } from 'lucide-react';
...
const AIRecipeGenerator = () => {
  ...
  const [customPrompt, setCustomPrompt] = useState('Generate a simple recipe using only');
  ...
  const exportRecipes = () => {
    try {
      ...
      link.href = url;
      link.download = `ai-recipes-${new Date().toISOString().split('T')[0]}.json`;
      link.click();
  ...
  const exportSingleRecipeAsText = (recipe) => {
    try {
      ...
      for (const line of lines) {
        if (line.match(/^(Recipe Name:|Title:|.*Recipe$)/i) || 
            (line.trim() && !line.match(/^\d+\./) && !line.match(/^(Ingredients|Instructions|Cooking Time|Servings):/i))) {
...
═══════════════════════════════════════════════════════════════
      ...
      let filename = recipeTitle.toLowerCase()
        .replace(/[^a-z0-9\s]/g, '')
        .replace(/\s+/g, '-')
        .substring(0, 30);
      
      if (!filename || filename === 'delicious-recipe') {
        filename = recipe.ingredients.length > 0 
          ? recipe.ingredients.slice(0, 2).join('-').toLowerCase().replace(/[^a-z0-9-]/g, '')
          : 'recipe';
      }
      
      filename = `${filename}-${new Date(recipe.createdAt).toISOString().split('T')[0]}.txt`;
      ...
      const htmlContent = `
        ...
        <body>
          <div class="header">
            ...
            ${recipe.isFavorite ? '<span class="favorite-badge">⭐ FAVORITE</span>' : ''}
          </div>
          
          <div class="meta-info">
            <div class="meta-row">
              <span class="meta-label">📅 Created:</span> ${formatDate(recipe.createdAt)}
            </div>
            <div class="meta-row">
              <span class="meta-label">🎯 Recipe Style:</span> ${recipe.prompt || 'Simple recipe with limited ingredients'}
            </div>
            <div class="meta-row">
  ...
  const exportSingleRecipeAsTextSimple = (recipe) => {
    try {
      const textContent = `
Recipe: ${recipe.ingredients.join(', ')}

Created: ${formatDate(recipe.createdAt)}
Prompt Used: ${recipe.prompt || 'Default prompt'}
...
Exported on: ${new Date().toLocaleString()}
      ...
      const filename = recipe.ingredients.length > 0 
        ? `recipe-${recipe.ingredients.slice(0, 2).join('-').toLowerCase().replace(/[^a-z0-9-]/g, '')}-${new Date(recipe.createdAt).toISOString().split('T')[0]}.txt`
        : `recipe-${new Date(recipe.createdAt).toISOString().split('T')[0]}.txt`;
      
      link.download = filename;
      link.click();
      URL.revokeObjectURL(url);
      showNotification('Recipe exported as text!', 'success');
    } catch (error) {
      showNotification('Failed to export recipe as text', 'error');
    }
  };
  ...
  const filteredRecipes = savedRecipes.filter(recipe =>
    ...
    (recipe.prompt && recipe.prompt.toLowerCase().includes(searchTerm.toLowerCase()))
  );
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const finalPrompt = `${customPrompt} ${ingredientList.join(', ')} and minimal common pantry items.
...
Format the response clearly and keep it concise. If the ingredients don't work well together, suggest the closest possible recipe or modifications.`;
      ...
      if (!response.ok) {
        ...
        throw new Error(`API Error: ${errorData.error?.message || errorData.error?.status || 'Failed to generate recipe'}`);
      }
      ...
      if (err.message.includes('API key')) {
        ...
        errorMessage = 'The AI had trouble with those ingredients. Try different combinations or check your spelling.';
      }
  ...
  const formatRecipeText = (text) => {
    ...
    const sections = text.split(/(?=\d+\.|Ingredients:|Instructions:|Cooking Time:|Servings:|Tips:|Recipe Name:|Title:)/i);
    ...
    return sections.map((section, index) => {
      ...
      if (trimmedSection.match(/^(Recipe Name:|Title:|Ingredients:|Instructions:|Cooking Time:|Servings:|Tips:)/i)) {
        return (
      ...
      if (trimmedSection.match(/^\d+\./)) {
        return (
          <div key={index} className="mb-1 pl-3">
            <p className="text-gray-700 text-xs leading-relaxed">{trimmedSection}</p>
          </div>
        );
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-red-50 to-pink-50 relative overflow-hidden">
      ...
      {/* Enhanced Notification System */}
      {notification && (
        <div className={`fixed top-4 right-4 px-6 py-4 rounded-2xl shadow-2xl z-50 border backdrop-blur-sm animate-in slide-in-from-right-5 duration-300 ${
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center mb-8 sm:mb-12">
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold bg-gradient-to-r from-orange-600 via-red-600 to-pink-600 bg-clip-text text-transparent mb-6 leading-tight">
              Simple Recipes with
              <br />
              <span className="text-3xl sm:text-4xl lg:text-5xl">Limited Ingredients</span>
            </h1>
            
            <p className="text-lg sm:text-xl text-gray-700 max-w-3xl mx-auto leading-relaxed mb-6">
              Create <span className="font-bold text-orange-600">simple recipes</span> using only the ingredients you specify. 
              Perfect for minimal cooking with maximum flavor!
            </p>
            ...
            {savedRecipes.length > 0 && (
              <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
                ...
                >
                  ...
                  <span>{showSavedRecipes ? 'Hide' : 'View'} Saved Recipes ({savedRecipes.length})</span>
                </button>
          ...
          {!GEMINI_API_KEY && (
            <div className="mb-6 bg-blue-50 border border-blue-200 rounded-2xl p-6">
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <h3 className="font-semibold text-blue-900 mb-2">Need a Gemini API Key?</h3>
                  <p className="text-blue-800 mb-3">
                    Get your free Gemini Pro API key to start generating recipes:
                  </p>
                  <ol className="text-blue-800 text-sm space-y-1 ml-4">
                    <li>1. Visit <a href="https://makersuite.google.com/app/apikey" target="_blank" rel="noopener noreferrer" className="underline font-medium">Google AI Studio</a></li>
                    <li>2. Sign in with your Google account</li>
                    <li>3. Click "Create API Key"</li>
                    <li>4. Copy the key and add it to your .env file as VITE_GEMINI_API_KEY</li>
                  </ol>
                </div>
              </div>
            </div>
              ...
              <div className="p-6 bg-gradient-to-r from-blue-400 to-purple-400">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-2xl font-bold text-white">Saved Recipes ({savedRecipes.length})</h2>
                  </div>
                  <button
                ...
                <div className="mt-4 flex items-center gap-2 text-white/90 text-sm">
                  ...
                  <span className="bg-white/20 px-2 py-1 rounded-full text-xs">
                    {Math.round((JSON.stringify(savedRecipes).length / 1024))} KB used
                  </span>
                </div>
              </div>
              ...
              <div className="p-6 max-h-96 overflow-y-auto">
                        ...
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex-1">
                            ...
                            <div className="flex flex-wrap gap-1 mb-2">
                              {savedRecipe.ingredients.slice(0, 2).map((ingredient, index) => (
                                <span
                                  key={index}
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            >
                              <Heart className={`w-3 h-3 ${savedRecipe.isFavorite ? 'fill-current' : ''}`} />
                            </button>
                            <button
                              onClick={() => exportSingleRecipeAsText(savedRecipe)}
                        ...
                        <div className="flex gap-1">
                          <button
                            onClick={() => {
                              ...
                              setIngredients(savedRecipe.ingredients.join(', '));
                ...
                {filteredRecipes.length === 0 && searchTerm && (
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-500">No recipes match your search "{searchTerm}"</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="p-6 sm:p-8 bg-gradient-to-br from-orange-400 via-red-400 to-pink-400 relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div className="flex-1">
                    <h2 className="text-2xl sm:text-3xl font-bold text-white">Custom Recipe Prompt</h2>
                    <p className="text-white/80 text-sm">Customize how you want your recipe generated</p>
                  </div>
                  {savedRecipes.length > 0 && (
                    <div className="flex items-center gap-2 bg-white/20 backdrop-blur-sm px-3 py-2 rounded-full">
                      <Save className="w-4 h-4 text-white" />
                      <span className="text-white text-sm font-medium">{savedRecipes.length}</span>
                    </div>
                  )}
                </div>
                ...
                {/* Custom Prompt Input */}
                <div className="mb-6">
                  <label className="text-white/90 text-sm font-medium mb-2 block">Recipe Style:</label>
                  <input
                    type="text"
                    value={customPrompt}
                  ...
                  <p className="text-white/70 text-xs mt-1">
                    Example: "Generate a simple recipe using only" or "Create a quick meal with just"
                  </p>
                </div>
              ...
              <div className="p-6 sm:p-8 bg-gradient-to-b from-gray-50/90 to-white/90 backdrop-blur-sm">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-gray-600 text-sm">Simple recipe with limited ingredients • Auto-saved</p>
                  </div>
                  <div className="flex items-center gap-2">
                  ...
                  <div className="bg-gradient-to-r from-orange-50/80 to-red-50/80 px-6 sm:px-8 py-5 border-t border-gray-100/50">
                    <div className="flex flex-wrap items-center justify-between gap-4">
                      <div className="flex items-center gap-2 bg-white/80 backdrop-blur-sm px-4 py-2 rounded-full shadow-sm border border-orange-100">
                        ...
                        <span className="font-semibold text-gray-700 text-sm">Quick & Simple</span>
                      </div>
                      <div className="flex items-center gap-2 bg-white/80 backdrop-blur-sm px-4 py-2 rounded-full shadow-sm border border-red-100">
                        ...
                        <span className="font-semibold text-gray-700 text-sm">Limited Ingredients</span>
                      </div>
                      <div className="flex items-center gap-2 bg-white/80 backdrop-blur-sm px-4 py-2 rounded-full shadow-sm border border-green-100">
                        ...
                        <span className="font-semibold text-gray-700 text-sm">Auto-Saved</span>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {loading && (
              <div className="p-6 sm:p-8 bg-gradient-to-b from-orange-50/90 to-white/90 backdrop-blur-sm border-t border-gray-100/50">
                <div className="text-center">
                  ...
                  <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-3">Creating your simple recipe...</h3>
                  <p className="text-gray-600 text-base sm:text-lg mb-4">Using only your specified ingredients with minimal additions</p>
            ...
            {!recipe && !loading && (
              <div className="p-6 sm:p-8 bg-gradient-to-b from-gray-50/90 to-white/90 backdrop-blur-sm border-t border-gray-100/50">
                <div className="text-center">
                  ...
                  <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-3">Ready to create a simple recipe?</h3>
                  <p className="text-gray-600 text-base sm:text-lg mb-6">Enter your ingredients and customize your prompt to generate recipes with limited ingredients!</p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-2xl mx-auto">
                    <div className="bg-white/80 backdrop-blur-sm p-4 rounded-2xl border border-gray-100 shadow-sm">
                      ...
                      <p className="text-gray-700 text-sm font-medium">Customize your prompt</p>
                    </div>
                    <div className="bg-white/80 backdrop-blur-sm p-4 rounded-2xl border border-gray-100 shadow-sm">
                    ...
                    <div className="bg-white/80 backdrop-blur-sm p-4 rounded-2xl border border-gray-100 shadow-sm">
                      ...
                      <p className="text-gray-700 text-sm font-medium">Auto-saved recipes!</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="text-center mt-8 sm:mt-12">
            <div className="inline-flex items-center gap-3 bg-white/70 backdrop-blur-sm px-8 py-4 rounded-full shadow-xl border border-white/50 hover:shadow-2xl transition-all duration-300">
              ...
              <span className="text-gray-700 font-semibold">Custom Prompts</span>
              <span className="text-gray-400">•</span>
              <span className="text-gray-700 font-semibold">Limited Ingredients</span>
              <span className="text-gray-400">•</span>
              <Save className="w-4 h-4 text-green-500" />
              <span className="text-gray-700 font-semibold">Persistent Storage</span>
            </div>
            
            <p className="text-gray-600 text-sm mt-4 max-w-md mx-auto">
              Create simple recipes using only the ingredients you specify with custom prompts.
              <br />
              <span className="text-green-600 font-medium">Recipes automatically saved and persist across sessions!</span>
            </p>
          </div>
        </div>
      </div>
